            ...
            synchronized (getTurnIndicator()) {
                try {
                    while (true) {
                        ...
                        Platform.runLater(() -> {
                            // Blue -> Red.
                            if (Game.getTurn() == PieceColor.RED && BoardTurnIndicator.getTurnIndicator()
                                                                                      .getFill() != red) {
                                FillTransition ft = new FillTransition(Duration.millis(2000),
                                                                       BoardTurnIndicator.getTurnIndicator(), blue,
                                                                       red);
                                ft.play();
                            }

                            // Red -> Blue.
                            else if (Game.getTurn() == PieceColor.BLUE && BoardTurnIndicator.getTurnIndicator()
                                                                                            .getFill() != blue) {
                                FillTransition ft = new FillTransition(Duration.millis(3000),
                                                                       BoardTurnIndicator.getTurnIndicator(), red,
                                                                       blue);
                                ft.play();
                            }
                        });
