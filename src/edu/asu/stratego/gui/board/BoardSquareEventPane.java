...
public class BoardSquareEventPane extends GridPane {
                ...
                if (squarePiece == null) {
                    ...
                    while (pieceType == null) {
                        ...
                        if (SetupPieces.getPieceCount(availTypes.get(randInt)) > 0)
                            ...
                            // There are no more available for that piecetype, remove it from the array so it won't
                            // be randomly generated again
                        else
    ...
    private void displayValidMoves(int pieceRow, int pieceCol) {
        ...
        for (Point validMove : validMoves) {
            Game.getBoard()
                .getSquare((int) validMove.getX(), (int) validMove.getY())
                .getEventPane()
                .getHover()
                .setImage(ImageConstants.HIGHLIGHT_VALID);
            Game.getBoard()
                .getSquare((int) validMove.getX(), (int) validMove.getY())
                .getEventPane()
                .getHover()
    ...
    private ArrayList<Point> computeValidMoves(int row, int col) {
        ...
        if (pieceType != PieceType.BOMB && pieceType != PieceType.FLAG) {
            // Negative Row (UP)
            for (int i = -1; i >= -max; --i) {
                if (isInBounds(row + i, col) && (!isLake(row + i, col) || (!isNullPiece(row + i,
                                                                                        col) && !isOpponentPiece(
                        row + i, col)))) {
            ...
            // Positive Col (RIGHT)
            for (int i = 1; i <= max; ++i) {
                if (isInBounds(row, col + i) && (!isLake(row, col + i) || (!isNullPiece(row,
                                                                                        col + i) && !isOpponentPiece(
                        row, col + i)))) {
